...
def check_verification_success(tab) -> Optional[VerificationStatus]:
    ...
    for status in VerificationStatus:
        if tab.ele(status.value):
            print(f"验证成功 - 已到达{status.name}页面")
            return status
    return None
...
def handle_turnstile(tab, max_retries: int = 3, retry_interval: tuple = (1, 2)) -> bool:
    ...
    try:
        ...
        # 超出最大重试次数
        print(f"验证失败 - 已达到最大重试次数 {max_retries}")
        print(
...
def sign_up_account(browser, tab):
    ...
    while True:
        try:
            ...
            print(f"验证码处理过程出错: {str(e)}")

    handle_turnstile(tab)
    wait_time = random.randint(3, 6)
    for i in range(wait_time):
        print(f"等待系统处理中... 剩余 {wait_time-i} 秒")
        time.sleep(1)
    ...
    try:
        ...
        if usage_ele:
            ...
            print(f"账户可用额度上限: {total_usage}")
            print(
                "请前往开源项目查看更多信息：https://github.com/chengazhen/cursor-auto-free"
            )
    except Exception as e:
        print(f"获取账户额度信息失败: {str(e)}")
...
def submit_token(token: str) -> bool:
    ...
    try:
        ...
        url = "https://testOrigin.com/submit_token.php"
        data = {
            "token": token,
